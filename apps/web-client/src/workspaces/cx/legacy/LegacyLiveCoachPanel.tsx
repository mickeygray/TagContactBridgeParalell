...
type LiveCoachPanelProps = {
  ...
  // Fires the resolved coach session id (or null) as the panel locks/clears the current call.
  // Lets a sibling surface (the new Focus Card cockpit) reuse this panel's session resolution
  // without duplicating the dashboard-match/lock lifecycle.
...
export function LiveCoachPanel({
  ...
  // Surface the resolved coach session id (reactively from the reducer) so a sibling
  // Focus Card cockpit can stream the same session. Additive — no effect on this panel.
  const resolvedSessionId = session?.id || null;
  React.useEffect(() => {
    onSessionResolved?.(resolvedSessionId);
  }, [resolvedSessionId, onSessionResolved]);
