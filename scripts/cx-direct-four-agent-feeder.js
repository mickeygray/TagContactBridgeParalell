...
const AGENTS = Object.freeze([
  ...
  // Phil is intentionally excluded from direct CX supply as of 2026-07-10.
  // His existing inventory was released and redistributed to the active agents.
]);
...
async function markPublishing(feed, rows, route, runId, now) {
  for (const row of rows) {
    await feed.updateOne(
      { _id: row.feedId },
      { $set: { status: "publishing", agent: route.key, agentEmail: route.email, campaignId: route.campaignId, lane: route.lane, externId: row.externId, runId, publishingAt: now, updatedAt: now }, $setOnInsert: { createdAt: now } },
      { upsert: true },
    );
  }
}
