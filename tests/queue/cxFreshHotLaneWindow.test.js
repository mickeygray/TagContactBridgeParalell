...
test("computeFreshHotLaneWindow uses previous 4pm LA before daily rollover", () => {
  ...
  assert.equal(window.rolloverHour, 16);
  assert.equal(window.windowStart.toISOString(), "2026-05-05T23:00:00.000Z");
  assert.equal(window.windowEnd.toISOString(), asOf.toISOString());
});

test("computeFreshHotLaneWindow uses same-day 4pm LA after rollover", () => {
  const asOf = new Date("2026-05-07T00:30:00.000Z"); // 5:30pm PDT on May 6
  const window = computeFreshHotLaneWindow(asOf);

  assert.equal(window.windowStart.toISOString(), "2026-05-06T23:00:00.000Z");
  assert.equal(window.windowEnd.toISOString(), asOf.toISOString());
});
