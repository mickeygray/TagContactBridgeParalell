...
test("control plane wires one gated runtime, durable credentials, safe controls, and cleanup", () => {
  ...
  assert.match(source, /productivityRebalanceEnabled: true/);
...
test("ordinary refill does not wait on the global productivity rebalance", () => {
  const source = fs.readFileSync(
    path.join(__dirname, "../../packages/shared-services/src/leadDeliveryService.js"),
    "utf8",
  );
  const postTop = source.match(
    /async function postTopOfQueueOnce[\s\S]*?function postTopOfQueue\(/,
  )?.[0] || "";
  assert.match(postTop, /withAgentPoolOperation|postPacketItem/);
  assert.doesNotMatch(postTop, /productivityRebalanceInFlight/);
});